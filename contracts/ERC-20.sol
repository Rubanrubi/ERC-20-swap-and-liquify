...
contract ERC20 is Context, IBEP20, IBEP20Metadata, Ownable {
    ...
    function _transfer(
        ...
        if(overMaxTokenBalance && sender != uniswapV2Pair){
            ...
            this.swapAndLiquify(contractTokenBalance);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) external {
        ...
        // swap tokens for ETH
        this.swapTokensForEth(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        // add liquidity to uniswap
        this.addLiquidity(otherHalf, newBalance);
